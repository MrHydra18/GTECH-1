...
//tous ce fichier est template et vise a rester template, NE PAS MODIFIER

//State Template pour la StateMachine
template <typename T>
class StateBase
{
public:
...
//Condition Template pour la StateMachine
template<typename T>
class Condition 
{
public:
...
//Transition Template pour la StateMachine 
template<typename T>
class Transition 
{
	int m_targetState;
...
//Conteneur et Chef d'orquestre pour la StateMachine
template<typename T>
class StateMachine {
private:
...
public:
	...
	void TransiteTo(T* type, int _newState) {
